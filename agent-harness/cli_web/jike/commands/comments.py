...
def comments_list(ctx, post_id, limit, load_more_key):
    ...
    with handle_errors(ctx.obj.get("json")):
        ...
        data = client.get_comments(
            post_id, limit=min(limit, 50), load_more_key=load_more_key
        )
        ...
        if ctx.obj.get("json"):
            ...
            for c in items:
                user = c.get("user", {})
                click.echo(
                    f"[{user.get('screenName', '?')}] {c.get('content', '')[:120]}"
                )
                click.echo(
                    f"  ❤ {c.get('likeCount', 0)}  💬 {c.get('replyCount', 0)}  — {c.get('id', '')}"
                )
                click.echo()
